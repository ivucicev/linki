...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (accounts.length === 0) return res.status(200).json({ ok: true, synced: 0, replies: 0, bounces: 0 });
  ...
  return res.status(200).json({ ok: true, accounts: accounts.length, replies, bounces });
}
